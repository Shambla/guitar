#!/usr/bin/env python3
"""
cd /Users/olivia2/Desktop/Stock_Strats/SLV_strat.py
python SLV_strat.py

Silver (futures) strategy on 1-hour candles:
...
- DXY (dollar) filter: silver is inversely correlated with the dollar. If DXY Close > DXY EMA 12
  (dollar strong), allow shorts only; if DXY below EMA 12 (dollar weak), allow longs only.
...
Default symbol is Yahoo Finance continuous silver futures: SI=F

Examples:
  cd /Users/olivia2/Desktop/Stock_Strats
  python3 -u SLV_strat.py --symbol SI=F --interval 60m --period 90d --backtest
  python3 -u SLV_strat.py --symbol SI=F --interval 60m --period 30d --loop --sleep-seconds 900
...
# Session boundaries (UTC) for silver — aligned with mechanical London/NY framework:
# - London session (silver): 3:00 AM – 8:00 AM ET = liquidity range; mark London High (LH) / London Low (LL).
#   In UTC (EST): 08:00–12:00 inclusive (3,4,5,6,7 AM ET). 8 AM ET = 13:00 UTC (NY open).
# - NY session open: 8:20 AM ET COMEX silver open; we use Open of 13:00 UTC bar (8 AM ET) as proxy.
...
class StrategyParams:
    ...
    # Dollar filter: silver is inversely correlated with the dollar.
    # If dollar Close > EMA 12 (dollar strong) -> bearish for silver (allow shorts, block longs).
    # Default UUP: Yahoo's ^DX often returns 404/no data; UUP ETF is reliable.
    dxy_symbol: str = "UUP"  # Yahoo symbol for dollar proxy (UUP = ETF; ^DX = index, often broken)
    dxy_ema_period: int = 12  # 60-minute candle EMA period for dollar
    use_dxy_filter: bool = True  # If False, ignore dollar filter (e.g. when fetch fails)
...
def _fetch_dxy(
    ...
    """
    Fetch dollar proxy OHLC (same interval/period as silver). Returns None on failure.
...
def _fetch_tenyr(
    ...
    """Fetch 10Y Treasury Yield OHLC with same interval/period as silver. Returns None on failure."""
    if yf is None:
        return None
    try:
        df = fetch_ohlcv(
...
def _merge_dxy_into_silver(silver_df: pd.DataFrame, dxy_df: pd.DataFrame, p: StrategyParams) -> pd.DataFrame:
    """
    Merge DXY Close and DXY EMA12 onto the silver DataFrame by Datetime.
    Adds column dxy_above_ema12: True when DXY Close > DXY EMA12 (dollar strong -> bearish for silver).
    """
    d = silver_df.copy()
    dxy = dxy_df.copy()
...
def _merge_tenyr_into_silver(silver_df: pd.DataFrame, tenyr_df: pd.DataFrame, p: StrategyParams) -> pd.DataFrame:
    """
    Merge 10Y Treasury Yield Close and EMA onto the silver DataFrame by Datetime.
...
def build_indicators(
    ...
    # DXY (dollar) filter: inverse relationship with silver. Dollar above EMA12 -> bearish for silver.
    if p.use_dxy_filter and dxy_df is not None and not dxy_df.empty:
        d = _merge_dxy_into_silver(d, dxy_df, p)
    else:
    ...
    if p.use_tenyr and tenyr_df is not None and not tenyr_df.empty:
        d = _merge_tenyr_into_silver(d, tenyr_df, p)
    else:
        d["tenyr_above_ema"] = False

    # Session levels (UTC): NY open 13:00; London 08:00–16:00. True/false for silver.
...
def latest_signal_summary(df: pd.DataFrame, p: StrategyParams, dxy_df: Optional[pd.DataFrame] = None) -> str:
    d = generate_signals(build_indicators(df, p, dxy_df=dxy_df))
    if d.empty:
        return "No data."
    last = d.iloc[-1]
...
def print_status_checklist(s: dict, title: str = "SILVER") -> None:
    """
...
def _format_status_line(s: dict) -> str:
    ...
    if s.get("dxy_close") is not None:
        dxy_sym = s.get("dxy_symbol_used") or "DXY"
        dxy_str = f" | {dxy_sym}={s['dxy_close']:.2f} above_EMA12={s.get('dxy_above_ema12')}"
    ...
    return (
        ...
        f"ATR={s.get('atr'):.4f} ({(s.get('atr_pct') or 0)*100:.2f}%)"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Silver futures 1H EMA/VWAP/ATR strategy")
    parser.add_argument("--symbol", type=str, default="SI=F", help="Yahoo symbol (default: SI=F)")
    ...
    parser.add_argument(
        ...
        type=str,
        default="SLV,SIL",
        help="Comma-separated fallback symbols if SI=F fails (default: SLV,SIL)",
    )
    ...
    parser.add_argument("--no-dxy-filter", action="store_true", help="Disable DXY (dollar) filter; trade silver without dollar confirmation")
    ...
    if args.loop:
        ...
        while True:
            ...
            if not args.no_cross_confirm:
                gold_df = fetch_ohlcv("GC=F", args.interval, args.period, max_cache_age_minutes=args.max_cache_age_minutes, fallback_symbols=["GLD", "IAU"])
            s = _status_payload(df, p, dxy_df=dxy_df, tenyr_df=tenyr_df, other_metal_df=gold_df)
    ...
    # Default: print True/False checklist for quick daily silver overview
    s = _status_payload(df, p, dxy_df=dxy_df, tenyr_df=tenyr_df, other_metal_df=gold_df)
    print_status_checklist(s, title="SILVER")
